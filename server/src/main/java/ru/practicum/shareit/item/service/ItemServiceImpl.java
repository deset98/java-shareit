...
public class ItemServiceImpl implements ItemService {
    ...
    public ItemServiceImpl(@Qualifier("ItemJpaRepository") final ItemRepository itemRepository,
                           ...
                           @Qualifier("BookingJpaRepository") final BookingRepository bookingRepository,
                           @Qualifier("CommentJpaRepository") final CommentRepository commentRepository,
                           @Qualifier("ItemRequestJpaRepository") final ItemRequestRepository itemRequestRepository) {
    ...
    public ItemDto addItem(long userId, ItemDto itemDto) {
        ...
        Optional.ofNullable(itemDto.getRequestId())
                .ifPresent(id ->
                        item.setRequest(itemRequestRepository.findById(id)
                                .orElseThrow(() -> new NotFoundException("ItemRequest id = " + id + " не существует")))
                );
    ...
    public List<ItemDto> searchItemsBySubstring(String substr) {
            return itemRepository.findAllBySubstring(substr).stream()
